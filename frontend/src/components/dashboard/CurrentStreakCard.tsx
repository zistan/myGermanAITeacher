...
export function CurrentStreakCard({ activity }: CurrentStreakCardProps) {
  const isActiveToday = activity.days_since_last_activity === 0;
    ...
    <Card>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="text-center p-3 bg-gray-50 rounded-lg">
          <div className="text-2xl font-bold text-gray-900">{activity.longest_streak_days}</div>
          <div className="text-xs text-gray-600 mt-1">Longest Streak</div>
        </div>
        <div className="text-center p-3 bg-gray-50 rounded-lg">
          <div className="text-2xl font-bold text-gray-900">{activity.total_active_days}</div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 text-center">
          ...
          <div className="text-sm font-medium text-gray-900">
            {activity.days_since_last_activity === 0
              ? 'Today'
              : activity.days_since_last_activity === 1
              ? 'Yesterday'
              : `${activity.days_since_last_activity} days ago`}
          </div>
        </div>
      )}

      {/* Motivation Message */}
      {activity.current_streak_days === 0 && activity.days_since_last_activity > 0 && (
